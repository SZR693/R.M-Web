...
const services = [
  ...
  { num: "02", icon: <Search size={24} aria-hidden="true" />, title: "Référencement SEO", desc: "Dominez Google dans votre région. Positionnez votre entreprise face aux clients qui vous cherchent.", items: ["Optimisation fiche Google", "Mots-clés locaux", "Stratégie de contenu SEO", "Netlinking ciblé"], path: "/seo" },
...
const faqs = [
  { q: "Combien de temps faut-il pour créer un site ?", a: "Pour un site vitrine classique, comptez entre 2 et 4 semaines. Pour un e-commerce ou un projet intégrant de l'IA, le délai moyen est de 6 à 8 semaines." },
  { q: "Qu'est-ce que le SEO Local concrètement ?", a: "Le SEO local permet à votre entreprise d'apparaître dans les premiers résultats de Google lorsqu'un client cherche vos services dans votre ville." },
  { q: "Comment l'IA peut-elle aider mon entreprise ?", a: "L'IA vous fait gagner du temps : chatbot 24h/24, automatisation de la prise de rendez-vous, liaison formulaires/facturation sans intervention humaine." },
...
export function Home() {
  ...
  return (
    <>
      <SEO
        title="R.M Web Design | Création de Sites Web & Design Digital sur mesure — Lyon"
        description="Agence web à Lyon spécialisée en création de sites web premium, SEO, SEA, SMA et automatisation IA. Design sur-mesure, performance et résultats mesurables."
        canonical="/"
        schema={homeSchema}
      />
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", textAlign: "center" }} className="max-md:!grid-cols-2 max-[480px]:!grid-cols-1">
            {stats.map((s) => (
              <div key={s.label} style={{ padding: "1.5rem", borderRight: "1px solid rgba(255,255,255,0.07)" }}>
                <div style={{ fontFamily: "var(--rm-serif)", fontSize: "3rem", fontWeight: 600, color: "var(--rm-gold-lt)", lineHeight: 1, marginBottom: "0.5rem" }} aria-label={s.num}>{s.num}</div>
            ...
            {services.map((s, i) => (
              <Reveal key={s.num} delay={i * 0.08}>
                <Link to={s.path} style={{ textDecoration: "none", color: "inherit", display: "block", height: "100%" }} aria-label={`Découvrir notre service : ${s.title}`}>
                  <div
                    style={{ background: "var(--rm-card)", border: "1px solid var(--rm-border)", borderRadius: "var(--rm-r)", padding: "2.5rem 2rem", position: "relative", overflow: "hidden", transition: "transform 0.35s, box-shadow 0.35s", height: "100%", display: "flex", flexDirection: "column" }}
          ...
          <Reveal>
            <div style={{ textAlign: "center", marginBottom: "4rem" }}>
              <div style={{ marginBottom: "2rem", display: "flex", justifyContent: "center" }}>
                <img src={logoSvg} alt="R.M Web Design" style={{ height: 220, width: "auto", objectFit: "contain", filter: isDark ? "drop-shadow(0 0 20px rgba(162, 119, 67, 0.3))" : "none", transition: "all 0.4s ease-in-out" }} />
              </div>
      ...
      <SectionPad style={{ background: "var(--rm-ink)", textAlign: "center" }}>
        <Container>
          <Reveal>
            <div style={{ marginBottom: "2rem", display: "flex", justifyContent: "center" }}>
              <img src={logoSvg} alt="R.M Web Design" style={{ height: 100, width: "auto", objectFit: "contain", filter: isDark ? "drop-shadow(0 0 15px rgba(162, 119, 67, 0.2))" : "none", transition: "all 0.4s ease-in-out" }} />
            </div>
        ...
        <div key={i} style={{ borderBottom: "1px solid var(--rm-border)", ...(i === 0 ? { borderTop: "1px solid var(--rm-border)" } : {}) }}>
          <h3 style={{ margin: 0 }}>
            ...
            >
              ...
              <div
                aria-hidden="true"
                style={{ width: 28, height: 28, flexShrink: 0, border: `1px solid ${open === i ? "var(--rm-gold)" : "var(--rm-border)"}`, borderRadius: "50%", display: "flex", alignItems: "center", justifyContent: "center", background: open === i ? "var(--rm-gold)" : "transparent", transition: "all 0.3s", color: open === i ? "#fff" : "var(--rm-gold)", fontSize: "1.2rem", lineHeight: 1 }}
              >
                {open === i ? "−" : "+"}
              </div>
            </button>
          ...
          <div
            ...
            style={{ maxHeight: open === i ? 200 : 0, overflow: "hidden", transition: "max-height 0.4s ease" }}
          >
